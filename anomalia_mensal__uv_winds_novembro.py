...
windsnetcdf4 = ('C:/Users/feder/OneDrive/ERA5/TCC/nc/winds_november.nc')
...
da_v[0,:,:].plot()
def calculate_november_wind_anomalies(da_u, da_v, reference_period=('1991-11-01', '2020-11-01')):
    # 1. Filtrar apenas dados de setembro
    nov_u = da_u.sel(valid_time=da_u['valid_time.month'] == 11)
    nov_v = da_v.sel(valid_time=da_v['valid_time.month'] == 11)
    
    # 2. Calcular climatologia (média 1991-2020)
    clim_u = nov_u.sel(valid_time=slice(reference_period[0], reference_period[1])).groupby('valid_time.month').mean()
    clim_v = nov_v.sel(valid_time=slice(reference_period[0], reference_period[1])).groupby('valid_time.month').mean()
    
    # 3. Calcular anomalias (dados observados - climatologia)
    anom_u = nov_u.groupby('valid_time.month') - clim_u
    anom_v = nov_v.groupby('valid_time.month') - clim_v
...
# Calcular anomalias
results = calculate_november_wind_anomalies(da_u, da_v)

# Selecionar anos específicos em 200 hPa
target_date = '2023-11-01'
anom = results['anom_v'].sel(valid_time=target_date, pressure_level=850) 
...
ax.set_title(f'Novembro {year}', fontsize=16)
