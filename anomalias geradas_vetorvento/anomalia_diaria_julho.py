...
windnetcdf4 = Dataset("C:/Users/Bolsista/Desktop/Federico/IC_CHICO/ERA5/Vetor vento (850 e 200 hPa)/era5_clim_july.nc")
windnetcdf4
...
clim_period_V = da_v.sel(valid_time=slice('1991-07-01', '2020-07-31')) # Sempre trocar de acordo com o mes
clim_period_U = da_u.sel(valid_time=slice('1991-07-01', '2020-07-31')) # Sempre trocar de acordo com o mes
...
anom_daily_V[1038]
anom_daily_U[1038]
anom_desired_V = anom_daily_V.sel(valid_time='2023-07-16')
anom_desired_U = anom_daily_U.sel(valid_time='2023-07-16')
...
wind_magnitude_2 = np.sqrt(anom_daily_U[1039]**2 + anom_daily_V[1039]**2)

mask = wind_magnitude > 9
...
magnitude_levels = np.linspace(4,18,15)


fig, ax = plt.subplots(1, 2, figsize = (10, 8), subplot_kw={'projection': ccrs.PlateCarree()})

ax[0].add_feature(cfeature.BORDERS, linestyle='-', linewidth=1)
ax[0].add_feature(cfeature.COASTLINE, linewidth=1)
#ax.set_title('Anomalia de vetor vento em 850 hPa - 02/05/2024 (1991-2020)')

wind_contour = ax[0].contourf(lon ,lat, wind_magnitude, cmap='jet',
                 levels=magnitude_levels,
                 extend='max', transform=ccrs.PlateCarree())
tiler = cimgt.GoogleTiles(style='satellite')
ax[0].add_image(tiler,6) # Aumentar o valor, aumenta a resolução

pular=8
ax[0].quiver(lon[::pular], lat[::pular], U_filtrado[::pular, ::pular].values,
...
ax[0].gridlines(draw_labels=dict(left=True, bottom=True, top=False, right=False),
              linewidth=1, color='gray', alpha=0.5, linestyle='--', transform=ccrs.PlateCarree())

ax[0].set_extent([-20, -80, -50, 0], crs=ccrs.PlateCarree())   
#ax.set_extent([-15, -105, 10, -90], crs=ccrs.PlateCarree())
ax[0].plot(-51.12, -30.02, marker='o', color='red', markersize=10, alpha=0.7)

#############################################################

ax[1].add_feature(cfeature.BORDERS, linestyle='-', linewidth=1)
ax[1].add_feature(cfeature.COASTLINE, linewidth=1)
#ax.set_title('Anomalia de vetor vento em 850 hPa - 02/05/2024 (1991-2020)')

wind_contour_2 = ax[1].contourf(wind_magnitude_2.longitude, wind_magnitude_2.latitude, wind_magnitude_2, 
                 cmap='jet', levels=magnitude_levels,
                 extend='max', transform=ccrs.PlateCarree())
  
tiler = cimgt.GoogleTiles(style='satellite')
ax[1].add_image(tiler,6) # Aumentar o valor, aumenta a resolução

pular=8
ax[1].quiver(lon[::pular], lat[::pular], U_filtrado[::pular, ::pular].values,
          V_filtrado[::pular, ::pular].values,
           transform=ccrs.PlateCarree(), scale=500, color='black',
           headwidth=2, headlength=3, linewidth=8, alpha=1)
ax[1].gridlines(draw_labels=dict(left=True, bottom=True, top=False, right=False),
              linewidth=1, color='gray', alpha=0.5, linestyle='--', transform=ccrs.PlateCarree())

ax[1].set_extent([-20, -80, -50, 0], crs=ccrs.PlateCarree())   
#ax.set_extent([-15, -105, 10, -90], crs=ccrs.PlateCarree())
ax[1].plot(-51.12, -30.02, marker='o', color='red', markersize=10, alpha=0.7)




# Barra de cores
colorbar_ticks = np.linspace(4,18,8)
